# HAPTIC KAZAAAN v0.10 — PAYOUT / HAPTICS FULL-CHAT AUDIT
...
v0.9.1までの通常/JP共通基礎式は維持しますが、SJPのみ以下を上書きします。

- 総PAYOUTは従来通り `BET × 100` で保存。
- 総枚数を100/250/400/600のamount blockへ分割。
- blockは刺激量を追加せず、同じPAYOUTをグループ化するだけ。
- SJP Power floor 78%→100%。
- SJP HARDNESS floor 0.72→1.00。
- 通常約95msのZEROに対し、SJP block内は約32ms→8ms、FINALE 4ms。
- block境界だけ約400ms→120msの明確なZERO BREAK。
- 最終blockはFINALEとしてPower floor 95→100 / HARDNESS 0.96→1.00。
- LIMITは比例変換 `Command × LIMIT / 100` のままで絶対上限を越えない。

したがってv0.10は「SJPだけ架空の追撃を加える」のではなく、**本来のSJP払い出し量を、通常より高Power・高HARDNESS・高Dutyの専用払い出し形式で消化する**仕様です。
